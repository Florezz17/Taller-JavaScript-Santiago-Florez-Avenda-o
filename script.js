...
function setupEvents() {
  generation.addEventListener('change', () => { state.filters.gen = generation.value; computeAndRender(); });
  sortBy.addEventListener('change', () => { state.filters.sortBy = sortBy.value; computeAndRender(); });
  ...
  favBtn.addEventListener('click', () => { state.filters.showFavs = !state.filters.showFavs; favBtn.classList.toggle('ring-2'); computeAndRender(); });
